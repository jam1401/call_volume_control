...
class VolumeObserver(private val context:Context){
    ...
    fun setVolumeByPercentage(volume:Double, showSystemUI:Boolean) {
        ...
        var maxVolume:Int = audioManager.getStreamMaxVolume(AudioManager.STREAM_VOICE_CALL)
        _volume = (round(volumePercentage * maxVolume)).toInt()

        audioManager.setStreamVolume(AudioManager.STREAM_VOICE_CALL, _volume, if (showSystemUI) FLAG_SHOW_UI else 0)
    }

    fun getVolume():Double {
        var currentVolume:Int = audioManager.getStreamVolume(AudioManager.STREAM_VOICE_CALL)
        var maxVolume:Int = audioManager.getStreamMaxVolume(AudioManager.STREAM_VOICE_CALL)
